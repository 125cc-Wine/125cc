...
module.exports = async function handler(req, res) {
  ...
  if (!SHEET_ID || !GOOGLE_CLIENT_EMAIL || !GOOGLE_PRIVATE_KEY) {
    return res.status(500).json({ error: "Faltan credenciales de Google en las variables de entorno." });
  }

  const { vino, bodega, tipo, precio, puntuacion, aroma, cuerpo, gusto, opinion } = req.body;
  ...
  try {
    ...
    const sheetResponse = await fetch(
      `https://sheets.googleapis.com/v4/spreadsheets/${SHEET_ID}/values/Degustaciones!A:K:append?valueInputOption=USER_ENTERED&insertDataOption=INSERT_ROWS`,
      {
        method: "POST",
        headers: {
