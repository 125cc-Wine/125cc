...
async function listProductos(req, res) {
  ...
  const { rows } = soloActivos
    ? await sql`
        SELECT id, nombre, categoria, unidad_venta, precio, costo, stock_actual, stock_minimo, activo, vino_ref
...
async function upsertProducto(req, res) {
  const { id, nombre, categoria, unidad_venta, precio, costo, stock_actual, stock_minimo, activo, vino_ref } = req.body || {};
  ...
  if (id) {
    const { rows } = await sql`
      ...
      WHERE id=${id}
      RETURNING id, nombre, categoria, unidad_venta, precio, costo, stock_actual, stock_minimo, activo, vino_ref`;
    if (!rows.length) return res.status(404).json({ error: "Producto no encontrado." });
    return res.status(200).json({ producto: rows[0] });
  }
  const { rows } = await sql`
    INSERT INTO productos (nombre, categoria, unidad_venta, precio, costo, stock_actual, stock_minimo, activo, vino_ref)
    VALUES (${nombre}, ${cat}, ${unidad}, ${precioNum}, ${costoNum}, ${stockAct}, ${stockMin}, ${act}, ${ref})
    RETURNING id, nombre, categoria, unidad_venta, precio, costo, stock_actual, stock_minimo, activo, vino_ref`;
