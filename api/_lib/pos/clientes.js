...
async function listClientes(req, res) {
  ...
  const { rows } = q
    ...
    : await sql`SELECT id, nombre, telefono, email, notas, cuit, razon_social, condicion_iva, created_at FROM clientes ORDER BY nombre LIMIT 100`;
  return res.status(200).json({ clientes: rows });
}

async function upsertCliente(req, res) {
  const { id, nombre, telefono, email, notas, cuit, razon_social, condicion_iva, creado_por } = req.body || {};
    ...
    if (id) {
      const { rows } = await sql`
        UPDATE clientes SET nombre=${nombreVal}, telefono=${tel}, email=${emailVal}, notas=${notasVal},
          cuit=${cuitVal}, razon_social=${razonSocialVal}, condicion_iva=${condIva}, updated_at=now()
        WHERE id=${id}
        RETURNING id, nombre, telefono, email, notas, cuit, razon_social, condicion_iva`;
      if (!rows.length) return res.status(404).json({ error: "Cliente no encontrado." });
      return res.status(200).json({ cliente: rows[0] });
    }
    const { rows } = await sql`
      INSERT INTO clientes (nombre, telefono, email, notas, cuit, razon_social, condicion_iva, creado_por)
      VALUES (${nombreVal}, ${tel}, ${emailVal}, ${notasVal}, ${cuitVal}, ${razonSocialVal}, ${condIva}, ${creado_por || null})
      RETURNING id, nombre, telefono, email, notas, cuit, razon_social, condicion_iva`;
